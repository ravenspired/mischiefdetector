...
import os.path
from os import path
def report_any_mischief():
    print("\nsendtoserver.py has been summoned.")

    #Detecting a screenshot to send
    if path.exists("offending_screenshot.png"):
        print("sendtoserver.py: mischievous screenshot found. Sending email to authorities.")
        report()
        print("sendtoserver.py: email sent. Moving on.")
    else:
        print("sendtoserver.py: no mischievous screenshot found. Continuing loop.")
